...
> This file is the single source of truth for all AI tools (Codex, GitHub Copilot, etc.) working in this repo.
...
7. [Supabase Operations (Codex)](#section-7--supabase-operations-Codex)
...
2. If the Supabase MCP isn't listed as a deferred tool at all, the
   server hasn't been registered with this Codex install yet.
   Point the user at Section 7 → "MCP Server Registration" for the
   one-time `Codex mcp add …` bootstrap, then restart the session.
...
This project uses **three long-lived branches**. All AI tools (Codex, GitHub Copilot, etc.) and developers MUST commit directly to the appropriate branch — do **not** create new session or feature branches.
...
- **Never create** `Codex/**`, `feature/**`, or any session branch — commit directly to `dev`. If the harness drops you on `Codex/*`, switch to `dev` before your first commit.
...
- **Promote to `main` when the user asks.** When explicitly instructed (e.g. "push to main", "merge to main"), Codex may fast-forward `main` to the current `dev` tip and push. Never rewrite `main`'s history — fast-forward only. If the push would not be a fast-forward, stop and surface the divergence. Keep `staging` fast-forwarded alongside so all three branches stay in sync. Without an explicit instruction, commit only to `dev`.
...
## Universal AI-Assisted Development Guidelines (Codex + Copilot + Others)
...
## 16. AI-Assisted Development Hygiene (Codex + Copilot + Others)
...
# SECTION 7 — Supabase Operations (Codex)

This repo is wired to a single Supabase project. Codex should prefer
...
Before the `mcp__supabase__*` tools can show up in a Codex
session, the Supabase MCP server has to be registered with the local
Codex install. This is a one-time bootstrap per machine/project
...
```
Codex mcp add --scope project --transport http supabase \
  "https://mcp.supabase.com/mcp?project_ref=vtarjrnqvcqbhoclvcur"
```

Then authenticate the server (interactive, user-only — Codex can't do
this step):
...
Verify by starting a fresh Codex session and confirming the
...
Agent Skills pack, which gives Codex extra ready-made instructions and
...
| `catalog-brainstorm` | Codex-generated catalog queries | `ANTHROPIC_API_KEY` |
...
- `ANTHROPIC_API_KEY` — Codex API key (Messages API).
