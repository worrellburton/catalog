...
const DeckViewV7: React.FC<DeckViewV7Props> = ({
  onSeeApp,
  onVisitWebsite,
  onBack,
  isLightMode,
  ...
  useEffect(() => {
    ...
    const slideMatch = hash.match(/^deck\/v7\/(\d+)$/);
    if (slideMatch) {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (idx >= 0) {
              window.history.replaceState(null, '', `#deck/v7/${idx + 1}`);
            }
          } else {
