...
export default function ModelMetrics({
  ...
  const items: MetricItem[] = [
    ...
    { label: 'Blended CPA', value: fmtCurrency(acqSummary.blendedCac), sub: `${fmtPercent(acqSummary.organicShare, 0)} organic`,
    ...
    { label: 'LTV : CPA', value: `${metrics.ltvCac.toFixed(1)}×`, sub: metrics.ltvCac >= 3 ? '≥3× healthy' : 'below 3×', tone: metrics.ltvCac >= 3 ? 'good' : 'warn',
      detail: `LTV ${fmtCurrency(metrics.ltv)} ÷ blended CPA ${fmtCurrency(acqSummary.blendedCac)}. ≥3× is the bar investors look for.` },
    { label: 'CPA payback', value: `${metrics.paybackMonths.toFixed(1)} mo`,
      detail: `Blended CPA ${fmtCurrency(acqSummary.blendedCac)} ÷ monthly gross profit per user (ARPU ${fmtCurrency(metrics.avgArpu)} × ${fmtPercent(econ.grossMargin, 0)} margin). Months to earn back the cost of an acquired user.` },
