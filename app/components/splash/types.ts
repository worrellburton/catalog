...
export interface SplashVariantProps {
  /** Downscaled (~5KB) image URLs from the live home feed. May be empty
   *  on a cold cache — variants should degrade gracefully (the host
   *  shows a logo-only fallback when there are zero images). */
  images: string[];
