      ...
      {isAppVisible && (
        <>
          <header>
            ...
            {/* Desktop-only gender chip cluster. The BottomBar pill
                that hosted these on mobile is hidden ≥769px (see
                bottom-bar.css), so the chips live in the header
                center on desktop only. Mobile keeps using the
                BottomBar's filter button + chips. */}
            <div className="header-filter-chips" role="tablist" aria-label="Catalog filter">
              <button
                role="tab"
                aria-selected={activeFilter === 'all'}
                className={`header-filter-chip ${activeFilter === 'all' ? 'active' : ''}`}
                onClick={() => handleGenderFilterChange('all')}
              >All</button>
              <button
                role="tab"
                aria-selected={activeFilter === 'women'}
                className={`header-filter-chip ${activeFilter === 'women' ? 'active' : ''}`}
                onClick={() => handleGenderFilterChange('women')}
              >Women</button>
              <button
                role="tab"
                aria-selected={activeFilter === 'men'}
                className={`header-filter-chip ${activeFilter === 'men' ? 'active' : ''}`}
