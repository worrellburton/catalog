...
import { getProductAffiliateProviders } from '~/utils/product-affiliates';
import { deriveTags } from '~/utils/derive-tags';
import { extractUrls, formatDateAdded } from '~/utils/data-format';
...
// Product tag heuristics (deriveTags) moved to app/utils/derive-tags.ts.

// Affiliate-provider helpers (AffiliateProvider, getProductAffiliateProviders,
// brand/retailer tables) moved to app/utils/product-affiliates.ts.
    ...
    <div className="admin-page">
      ...
      {activeTab === 'looks' && (
        ...
        <div className="admin-tabs" style={{ marginBottom: 12 }}>
          {/* Status rule: published === Live, not-published === Inactive.
              The leading dot is the indicator (green=live, amber=inactive,
              red=failed); the underlying filter keys stay published/
              unpublished/failed so URLs + queries don't change. */}
          <button
            className={`admin-tab ${looksFilter === 'published' ? 'active' : ''}`}
            onClick={() => setLooksFilter('published')}
            title="Live — published looks shown on the public feed"
          >
            <span className="admin-status-dot admin-status-dot--live" aria-hidden="true" />
            Live
          ...
          <button
            ...
            title="Inactive — not published (generated but not yet live)"
          >
            <span className="admin-status-dot admin-status-dot--inactive" aria-hidden="true" />
            Inactive
          ...
          >
            <span className="admin-status-dot admin-status-dot--failed" aria-hidden="true" />
            Failed
