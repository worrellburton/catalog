...
//
// Auth: verify_jwt=true (Supabase gateway validates anon key or user JWT).
...
function errorRes(message: string, status = 400) { return jsonRes({ success: false, error: message }, status); }
...
interface RainforestProduct {
  asin?: string; title?: string; brand?: string; description?: string; link?: string;
  main_image?: RainforestImage; images?: RainforestImage[];
  buybox_winner?: { price?: RainforestPrice };
  categories?: { name: string }[];
}
interface RainforestSearchResult {
  asin?: string; title?: string; brand?: string; link?: string; image?: string;
  rating?: number; ratings_total?: number; price?: RainforestPrice; prices?: RainforestPrice[];
}
interface NormalizedProduct {
  asin: string | null; name: string; brand: string | null; price: string | null;
  currency: string | null; description: string | null; url: string | null;
  image_url: string | null; images: string[]; categories: string[];
...
function normalizeProduct(p: RainforestProduct): NormalizedProduct {
  ...
  return {
    asin: p.asin ?? null, name: p.title ?? '', brand: p.brand ?? null,
    price: priceString(price), currency: price?.currency ?? null,
    description: p.description ?? null, url: p.link ?? null, image_url: mainImage,
...
function normalizeSearchResult(r: RainforestSearchResult): NormalizedProduct {
  ...
  return {
    asin: r.asin ?? null, name: r.title ?? '', brand: r.brand ?? null,
    price: priceString(price), currency: price?.currency ?? null,
    description: null, url: r.link ?? null, image_url: r.image ?? null,
    images: r.image ? [r.image] : [], categories: [],
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method !== 'POST')    return errorRes('POST required', 405);

  const apiKey = Deno.env.get('RAINFOREST_API_KEY');
  if (!apiKey) return errorRes('RAINFOREST_API_KEY is not configured. Set it in project secrets.', 500);
  ...
  if (!asin && !url && !keyword) return errorRes('Provide asin, url, or keyword');

  if (keyword) {
    const params = new URLSearchParams({ api_key: apiKey, type: 'search', amazon_domain: amazonDomain, search_term: keyword });
    let res: Response;
    try { res = await fetch(`https://api.rainforestapi.com/request?${params.toString()}`); }
    catch (err) { return errorRes(`Rainforest request failed: ${err instanceof Error ? err.message : String(err)}`, 502); }
    if (!res.ok) { const text = await res.text(); return errorRes(`Rainforest returned ${res.status}: ${text.slice(0, 500)}`, 502); }
  ...
  const params = new URLSearchParams({ api_key: apiKey, type: 'product', amazon_domain: amazonDomain });
