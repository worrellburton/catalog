-- search_products — fall back to products.primary_video_url (product-primary
-- model) when a product has no product_creative row.
--
-- Root cause of "Nothing in Latte Glasses yet": the RPC built video_url ONLY
-- from the product_creative table via the lateral join. Product-primary SKUs
-- (e.g. the Fellow Latte Glasses) carry their renderable clip in
-- products.primary_video_url and have NO product_creative row, so search
-- returned them as is_placeholder=true / video_url=null. The consumer feed
-- filters out video-less placeholders → the catalog renders empty even though
-- the product is active and has a primary video.
--
-- Fix: in the final projection, coalesce the creative fields with the product's
-- primary video/image/url (mirrors getHomeFeed's product→tile mapping). A row
-- is only a placeholder when it has NEITHER a creative NOR a primary video.
...
as $function$
  ...
  select
    ...
    p.id                                                    as product_id,
    c.id                                                    as creative_id,
    -- placeholder only when there's neither a creative nor a primary video
    (c.id is null and p.primary_video_url is null)          as is_placeholder,
    coalesce(c.video_url, p.primary_video_url)              as video_url,
    coalesce(c.thumbnail_url, p.primary_image_url, p.image_url) as thumbnail_url,
    coalesce(c.affiliate_url, p.url)                        as affiliate_url,
    c.duration_seconds,
    coalesce(c.is_elite, p.is_elite, false)                 as is_elite,
