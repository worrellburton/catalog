-- search_products — product tiles render strictly from products.primary_video_url.
--
-- Supersedes the product_creative fallback added in
-- 20260601000004_search_product_primary_video_fallback. That fallback is now
-- redundant: 20260601000005 backfilled legacy creatives into primary_video_url
-- and added trg_promote_creative_to_primary_video so any future creative that
-- goes live/done is promoted automatically. So a product's renderable clip is
-- always primary_video_url.
--
-- New contract (matches getHomeFeed): video_url = products.primary_video_url.
-- A product with no primary_video_url is a placeholder (is_placeholder=true,
-- video_url=null) and the consumer feed filters it out — "no primary video,
-- not on the feed". The product_creative lateral join is dropped entirely.
