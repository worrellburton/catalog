-- Search V7 Phase 1.1 — subcategory granularity tier.
--
-- Problem from V7 testing: "blue jeans" also returned shorts, because both are
-- product_taxonomy.category = 'bottoms' and the category filter can't tell jeans
-- from shorts. Same shape for "leather boots" vs sandals, "tee" vs "polo", etc.
--
-- Fix: when the query names a SPECIFIC sub-type (jeans, shorts, boots, sneakers,
-- tee, polo, maxi…), add a subcategory tier as the TOP sort key. Admission still
-- happens at the category level (grid stays full — graceful degradation), but the
-- exact sub-type floats above its siblings. Match is checked against BOTH
-- taxonomy.subcategory AND the product name (names like "Kate Jean" / "Era Low
-- Top Sneaker" carry the sub-type even when subcategory is sparse).
--
-- Sort precedence on the structured path becomes:
--   subcategory match  >  color-family match  >  residual BM25  >  conversion_score
--
-- Vibe path unchanged.
...
declare
  ...
  v_tax    text[];
  v_color  text;
  v_subcat text;  -- regex for a specific sub-type, or NULL
begin
  v_types := case
