-- Search V7 → canonical. Promote the structured-facet-routing logic (validated
-- as search_products_v7) into the canonical search_products, and drop the shadow.
--
-- After this migration the `search` edge fn's default RPC (search_products) IS
-- the V7 logic — no variant flag needed. Rollback: re-apply 20260603000002
-- (the prior V6.1 search_products) and this function reverts.
--
-- Behaviour (proven via tests/search/eval-relevance.mjs, 12/12):
--   * category query (product noun) -> structured path: hard-filter to category
--     via (type OR taxonomy.category), tier subcategory > color-family > BM25 >
--     conversion_score. No cross-category leak possible.
--   * vibe/open query -> V6.1 hybrid path verbatim (byte-identical to prior).
...
-- Remove the shadow now that its logic is canonical.
drop function if exists public.search_products_v7(vector(384), text, integer, text, uuid[]);
