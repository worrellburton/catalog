-- Search V8.1 (SHADOW) — add the structured aesthetic[] tier to the aesthetic route.
--
-- styling_metadata.aesthetic (controlled, apparel-only, set in this migration's
-- companion data update) now drives ranking: items explicitly tagged with the
-- detected aesthetic are ALWAYS admitted and rank above mere keyword-expansion
-- matches. product_occasions_text does NOT read this key, so the live
-- search_products is unaffected. Only the aesthetic branch changes vs V8.
