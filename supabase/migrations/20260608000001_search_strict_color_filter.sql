-- Search: strict color filter for CATEGORY queries.
--
-- Until now color was a SOFT tier in the category route — "black shoes"
-- ranked black first but still filled the grid with the rest of the footwear
-- category (which read as "white shoes showing for a black query"). Make
-- color a HARD filter instead: when the query names a color, the category
-- route only admits products whose product_taxonomy.color matches that family.
--
-- Scope is deliberately the CATEGORY route ONLY. The aesthetic route
-- (quiet luxury, streetwear, …) and the vibe/hybrid route never reference
-- v_color, so this change cannot affect them — vibe and semantic search are
-- byte-identical to V8. Verified: only the one `and (v_color is null …)`
-- line in cat_base differs from 20260605000009.
--
-- Consequence (accepted): a color the catalog doesn't carry returns an empty
-- grid (e.g. "black jacket" when the only jacket is Pearl) → the client's
-- EmptyCatalogState / "I want this catalog" CTA handles it. Strict means
-- strict; precision over a padded grid.
--
-- Matching is on product_taxonomy.color (clean, structured) NOT the name —
-- names false-positive on brands ("Black Suede Studio" is not a black shoe).
-- Rollback: re-apply 20260605000009.
