-- Search: make the category route's `type` match CASE-INSENSITIVE.
--
-- Bug: the category branch admits candidates via
--   (p.type = any(v_types) or lower(p.product_taxonomy->>'category') = any(v_tax))
-- but v_types held Capitalized literals ('Top','Shirt','Shoes','Pants',…) while
-- the catalog stores `type` in lowercase ('shirt','shoes','pants'). Postgres `=`
-- is case-sensitive, so the type predicate matched almost nothing and the pool
-- survived ONLY on the sparsely-populated product_taxonomy.category — collapsing
-- every garment-noun query to a 5–9 item sliver (67 shirts → 5, 84 shoes → 8).
--
-- This starved BOTH consumer catalog search AND the Style Up stylist (which
-- reaches the catalog through style_slot_search → search_products), so every
-- stylist bot surfaced the same handful of pieces and "more shoes/shirts"
-- exhausted after one tap.
--
-- Fix: lowercase the v_types literals (v_types is used ONLY in that one
-- predicate) and compare lower(p.type). Both sides lowercase → case-insensitive.
-- Purely ADDITIVE: every row that matched before still matches; lowercase-typed
-- rows now match too. No row loses a match, so no caller can regress. The
-- aesthetic route and the default hybrid route are byte-for-byte unchanged.
-- Rollback: re-apply 20260707000000_search_price_filter.sql.
...
begin
  -- Lowercase literals: matched case-insensitively against lower(p.type) below.
  v_types := case
  ...
  if v_types is not null or v_tax is not null then
    ...
    with cat_base as (
      ...
      where p.is_active = true
        ...
        -- Case-insensitive type match (v_types is lowercased above) — the catalog
        -- stores `type` lowercase, so an exact-case `=` matched almost nothing.
        and (lower(p.type) = any(v_types) or lower(p.product_taxonomy->>'category') = any(v_tax))
