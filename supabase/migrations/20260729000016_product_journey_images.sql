-- product_journey() now returns the IMAGES at each stage, not just counts:
--   kept  = the re-hosted gallery the product actually uses
--   raw   = the merchant's originals (images_raw, set-once)
--   video/poster on the creative step
--
-- kept and raw cannot be diffed by URL - kept images are re-hosted to our
-- storage under /products/<id>/N.ext while raw are merchant URLs - so the UI
-- shows both and reports pruned_count as the difference in length.
--
-- Supersedes the definition in 20260729000014; body is otherwise unchanged.
...
begin
  ...
  v_kept := case when jsonb_typeof(p.images) = 'array' then p.images else '[]'::jsonb end;
  v_raw  := case when jsonb_typeof(p.images_raw) = 'array' then p.images_raw else '[]'::jsonb end;
  ...
  v_steps := jsonb_build_array(
    ...
    jsonb_build_object('key','scraped','label','Scraped','at',p.scraped_at,
      'detail', case when p.scrape_status = 'done'
                     then jsonb_array_length(v_kept) || ' images · '
    ...
    jsonb_build_object('key','verified','label','Images verified','at',p.image_verified_at,
      ...
      'kept', v_kept, 'raw', v_raw,
      'pruned_count', greatest(jsonb_array_length(v_raw) - jsonb_array_length(v_kept), 0)),
    ...
    jsonb_build_object('key','creative','label','Creative generated','at',p.primary_video_generated_at,
      ...
      'video', p.primary_video_url, 'poster', p.primary_video_poster_url),
